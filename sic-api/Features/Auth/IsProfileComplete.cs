...
public static class IsProfileComplete
{
    ...
    public sealed class Handler(SicDbContext dbContext) : IRequestHandler<Query, bool>
    {
        ...
        {
            return await dbContext.SuProfiles.Where(x => x.KeycloakUserId == request.User.GetKeycloakUserId()).AnyAsync(cancellationToken);
        }
    }
}
