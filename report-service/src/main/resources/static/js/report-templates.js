(() => {
	const domReady = () => {
		const dialog = document.getElementById('delete-report-template-modal');
		if (!dialog) {
			return;
		}
		...
		const defaultTitle = title?.textContent || 'Delete Report Template';
		const defaultDescription = description?.textContent || 'Delete this report template? This action cannot be undone.';
		...
		document.addEventListener('click', (event) => {
			...
			const label = trigger.getAttribute('data-delete-label') || 'this report template';
