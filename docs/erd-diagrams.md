...
- ใช้ `%%{init: {"er": {...fontSize: 16...}}}%%` วางไว้ **หลัง** `erDiagram` เท่านั้น (ถ้าวางก่อนหน้า mermaid จะไม่นำไปใช้เงียบๆ ทดสอบแล้วจริง)
- entity ที่ไม่มีความสัมพันธ์ (FK) กับใครเลยในโค้ดจริง (เช่น `su_message`, `su_notification`, `db_title`, `db_parameter`) จะไม่วาดเป็นกล่องในภาพ แต่ระบุเป็น comment ในไฟล์แทน เพื่อไม่ให้เกิดกล่องลอยที่ทำให้ภาพว่างเปล่าโดยไม่จำเป็น
