...
import { SicTaskComponent, SicCalendarTask, SicTaskConfig } from '../../../../core/component/sic-task/sic-task.component';
// <<< เปลี่ยน import ให้ใช้ชื่อ component ที่ถูกต้อง >>>
import { SicGanttchartComponent, GanttTask } from '../../../../core/component/sic-ganttchart/ganttchart.component';
...
@Component({
    ...
    imports: [
        ...
        RouterModule,
        SicTaskComponent,
        SicGanttchartComponent,   // ใช้ชื่อที่ถูกต้อง
    ],
...
export class Pmdt02Component implements OnInit {
    ...
    // ===== Task Config สำหรับ SicTaskComponent =====
    taskConfig = computed<SicTaskConfig>(() => ({
        api: `/api/pm/projects/${this.currentPhaseId()}/tasks`,
        id: 'id',
        startDateParam: 'startDate',
        endDateParam: 'endDate',
        saveApi: `/api/pm/tasks`,
        saveMethod: 'POST',
    ...
    // ===== Computed สำหรับแปลง tasks เป็น GanttTask =====
  ...
  p.milestones?.forEach((ms) => {
    ms.workPackages?.forEach((wp) => {
      wp.tasks?.forEach((t) => {
        // ✅ ข้าม Task ที่ไม่มี startDate หรือ endDate
                ...
                this.taskService.deleteTask(task.id).subscribe({
                    next: () => {
                        ...
                        if (current?.milestones) {
                            current.milestones = current.milestones.map((m) => ({
                                ...m,
                                workPackages: m.workPackages?.map((wp) => ({
                                    ...wp,
